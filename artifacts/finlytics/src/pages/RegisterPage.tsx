...
export default function RegisterPage() {
  ...
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<RegisterFormValues>({
    ...
    defaultValues: { name: "", email: "", password: "", organizationName: "", terms: false },
  })
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    setIsLoading(true)
    try {
      const response = await api.post<{ token: string; user: any; organization: any }>("/auth/register", data)
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/40 px-4 py-12">
      <Card className="w-full max-w-lg shadow-lg border-none bg-background">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>Join thousands of businesses managing their finances with AI</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              <Input id="organizationName" placeholder="Acme Corp" {...register("organizationName")} className={errors.organizationName ? "border-destructive" : ""} />
            ...
            <div className="flex items-center space-x-2">
              <Checkbox id="terms" checked={termsValue} onCheckedChange={(checked) => setValue("terms", checked as boolean)} />
        ...
        <CardFooter className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Link href="/login" className="text-sm text-primary hover:underline font-medium">Sign in</Link>
        </CardFooter>
      </Card>
    </div>
  )
