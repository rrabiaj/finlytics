...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    setIsLoading(true)
    try {
      const response = await api.post<{ token: string; user: any; organization: any }>("/auth/login", data)
    ...
    <div className="min-h-screen flex items-center justify-center bg-muted/40 px-4">
      <Card className="w-full max-w-md shadow-lg border-none bg-background">
        ...
        <CardFooter className="flex flex-wrap items-center justify-center gap-2">
          ...
          <Link href="/register" className="text-sm text-primary hover:underline font-medium">Sign up</Link>
        </CardFooter>
      </Card>
    </div>
  )
